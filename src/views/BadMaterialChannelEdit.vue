...
import {badMaterialUpdateService,badMaterialAddService} from "@/api/material"
import {ref} from "vue";
...
const onSubmit=async ()=>{
  ...
  if (idEdit){
    ...
    const response=await badMaterialUpdateService(fromModel.value)

    console.log(response.data)

  }else {
    await form.value.validate()
    const response=await badMaterialAddService(fromModel.value)
