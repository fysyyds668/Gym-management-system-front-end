...
import {badMaterialAGetService,badMaterialDeleteService} from "@/api/material"
import BadMaterialChannelEdit from "@/views/BadMaterialChannelEdit.vue"
...
const handleDelete= async ({row}:{row:any})=>{
  ...
  const response=await badMaterialDeleteService(row.materialId)
...
const fetchData=async (page,size)=>{
  try {
    ...
    const response=await badMaterialAGetService({materialName,page,size})
...
<template>
  <div class="frame">
    <el-card class="card1">
      ...
      <el-row>
        <div>
          <el-breadcrumb>
            ...
            <el-breadcrumb-item><span>坏器材</span></el-breadcrumb-item>
          </el-breadcrumb>
        </div>
      </el-row>
      ...
      <BadMaterialChannelEdit ref="dialog" @success="onSuccess">

      </BadMaterialChannelEdit>
