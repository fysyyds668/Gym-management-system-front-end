...
import {courseGetPageService,courseDeleteService} from "@/api/course"
import courseChannelEdit from "@/views/courseChannelEdit.vue"
...
const handleDelete= async ({row}:{row:any})=>{
  ...
  const response=await courseDeleteService(row.id)
...
const fetchData=async (page,size)=>{
  try {
    loading.value=true
    const courseName=searchText.value
    const response=await courseGetPageService({courseName,page,size})
  ...
  <div class="frame">
    <el-card class="card1">
      ...
      <el-row>
        <div>
          <el-breadcrumb>
            ...
            <el-breadcrumb-item><span>课程管理</span></el-breadcrumb-item>
          </el-breadcrumb>
        </div>
      </el-row>
      ...
      <el-row :gutter="15" style="top: 15px">
        <el-col :span="6">
          <el-input v-model="searchText" placeholder="请输入要查找的课程名"></el-input>
        </el-col>
        <el-col :span="8">
      ...
      <course-channel-edit ref="dialog" @success="onSuccess">
